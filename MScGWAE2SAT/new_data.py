...
from preprocessing import new_cnf_to_adj, pad_smaller_instance


class SATDataset(Dataset):
    def __init__(self, directory, labels, max_shape):
        self.labels = labels
...
def get_train_val_dataloader(path, batch_size, max_shape, val_size=0, labels=None):
    if labels is None:
        labels = permutation(os.listdir(path))
    cut = int(len(labels) * val_size)
    training_ds = SATDataset(path, labels[cut:], max_shape)
    val_ds = SATDataset(path, labels[:cut], max_shape)
    training_dl = DataLoader(training_ds, batch_size=batch_size, shuffle=True)
    val_dl = DataLoader(val_ds, batch_size=batch_size)
    return training_dl, val_dl


def get_max_shape(path):    
    max_rows = 0
    max_cols = 0
...
def get_dataloader_with_names(path, batch_size, max_shape, labels=None):
    if labels is None:
        labels = os.listdir(path)
    dataset = SATDataset(path, labels, max_shape)
