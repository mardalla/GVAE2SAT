...
def generate(state_dict, src, dest, batch_size, device, mask, regen_factor,
             enc_size, dec_size, num_gnn, num_expansions, dropout, num_gen, reduced,
    ...
    original_cnfs, labels = get_dataloader_with_names(src, batch_size, max_shape, reduced, labels)
    ...
    with torch.no_grad():
        ...
        for idx, data in enumerate(original_cnfs):
            ...
            for jdx, datum in enumerate(data):
                ...
                gen_counter = num_gen if reduced else int(num_gen / 2 + .5)
                    
                for generatum in range(gen_counter):
                    ...
                    if mask is not None:
                        ...
                        if reduced:
                            if "clause" in mask:
                                new_adj = mask_clauses(new_adj, num_clauses)
                            if "var" in mask:
                                new_adj = mask_variables(new_adj, num_var)
                    ...
                    if reduced:
                        ...
                        corner = new_adj[:num_clauses, num_clauses:]
                        adj_to_cnf(corner, original_num*regen_factor,
                                   join(dest, f"{original_name[:-4]}-{generatum}.cnf"),
                                   pol_ratio, indices, comments)

                        corner = new_adj[num_clauses:, :num_clauses]
                        adj_to_cnf(corner, original_num*regen_factor,
                                   join(dest, f"{original_name[:-4]}-{generatum+gen_counter}.cnf"),
                                   pol_ratio, indices, comments)
