...
def main():
    ...
    for forcing in [False, True]:
        for masking in [None, "clause", "var", ["clause", "var"]]:
            comp = f"par{forcing}_mask{masking}"
            comparisons += [comp]
            vgae = get_vgae(max_shape, 32, device,
                            128, 256, 2, 3, .4)
            optim = torch.optim.Adam(vgae.parameters(), .003, weight_decay=.1)
            ...
            generate(model, args.src,
                     f"formulas_{comp}", 16,
                     device, masking, 1, 128, 256, 2, 3, .4, 10,
                     test_labels, forcing)
