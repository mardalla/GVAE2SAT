...
def main():
    ...
    comparisons = []
    for lr in [.3, .01, .001, .0003]:
        for wd in [.01, .1, .5]:
            comp = f"lr{lr}_wd{wd}"
            comparisons += [comp]
            vgae = get_vgae(max_shape, 32, device,
                            128, 256, 2, 3, .4)
            optim = torch.optim.Adam(vgae.parameters(), lr, weight_decay=wd)
            ...
            generate(model, args.src,
                     f"formulas_{comp}", 16,
                     device, "clause", 1, 128, 256, 2, 3, .4, 10, test_labels, True)
