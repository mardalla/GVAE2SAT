...
def main():
    ...
    for cnf in os.listdir(args.src):
        try:
            ...
            gwae2sat = get_average(gwae2sat_allo, cnf, "gwae2sat_allo", '-')
            df = pd.concat([df, gwae2sat])
            gwae2sat = get_average(gwae2sat_auto, cnf, "gwae2sat_auto", '-')
            df = pd.concat([df, gwae2sat])
        except:
            continue
    ...
    tsne_result_df.to_csv("tsne_results.csv", index=False)
