...
def main():
    ...
    comparisons = []
    for wd in [.1, .3]:
        for beta in [.01, .001, .0001]:
            for forcing in [False, True]:
                comp = f"wd{wd}_beta{beta}_forcing{forcing}"
                comparisons += [comp]
                vgae = get_vgae(max_shape, 32, device,
                                128, 256, 2, 3, .4)
                optim = torch.optim.Adam(vgae.parameters(), .003, weight_decay=wd)
                mae_loss = torch.nn.L1Loss()

                model = train(vgae, train_dl, val_dl, 20, 16,
                              optim, mae_loss, 9, beta, "models/", device)

                generate(model, args.src,
                         f"formulas_{comp}", 16,
                         device, None, 1, 128, 256, 2, 3, .4, 10, test_labels, forcing)
